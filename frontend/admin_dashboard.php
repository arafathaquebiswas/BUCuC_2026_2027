...
if (isset($_SESSION['admin_role']) && $_SESSION['admin_role'] === 'main_admin') {
    header("Location: super_admin_dashboard.php" . (isset($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : ''));
    exit();
}
...
function getDashboardStats()
{
    try {
        ...
        if ($result) {
            return [
                'total_members' => (int) $result['totalmembers'] + $approvedCount, // Add real-time approved members
